...
public class TDASHOptions extends TProfile {
    ...
    public List<Transco> commandBuilder(String input, String output,
            boolean live, String title) throws MCASException {
        ...
        for (TLevel level : getLevels()) {
            ...
            if ((getvCodec() != "") && (!live)) {
                ...
                cmd += " -g 24 -r 24 -vf scale=\"" + level.getWidth()
                        + ":trunc(ow/a/2)*2\"";
                cmd += " -b:v " + level.getMaxRate()
                        + "k -bufsize 10000k -maxrate " + level.getMaxRate()
                        + "k";
                cmd += " -map 0:0 -f segment -segment_time " + getSegDuration()
                        + " " + output + File.separator;
                cmd += MediaUtils.fileNameMakerByLevel(title, getName(),
                        level.getName())
                        + "_video_%d.mp4";
            } else if (getvCodec() != "") {
                cmd += " -vcodec " + getvCodec() + " " + getAdditionalFlags();
                cmd += " -g 24 -r 24 -b:v " + level.getMaxRate()
                        + "k -bufsize 10000k -maxrate " + level.getMaxRate()
                        + "k";
                cmd += " -vf scale=\"" + level.getWidth()
                        + ":trunc(ow/a/2)*2\"";
                cmd += " -map 0:0 -f segment -segment_time " + getSegDuration()
                        + " " + output + File.separator;
                cmd += MediaUtils.fileNameMakerByLevel(title, getName(),
            ...
            if ((getaCodec() != "") && (!live)) {
                cmd += " -c:a " + getaCodec();
                cmd += " -ac " + level.getaChannels() + " -b:a "
                        + level.getaBitrate() + "k ";
                cmd += " -map 0:1 -f segment -segment_time " + getSegDuration()
                        + " " + output + File.separator;
                cmd += MediaUtils.fileNameMakerByLevel(title, getName(), ""
                        + level.getaBitrate())
                        + "_audio_%d.mp4";
    ...
    @Override
    public void processManifest(Transco transco, String title)
    ...
    @Override
    public List<SimpleEntry<String, Integer>> getUris(URI destination,
            String title, boolean live) throws MCASException {
        List<SimpleEntry<String, Integer>> uris = new ArrayList<SimpleEntry<String, Integer>>();
        try {
            URI dst = new URI(destination.getScheme(), destination.getHost(),
                    destination.getPath()
                            + "/"
                            + MediaUtils.fileNameMakerByProfile(title,
                                    getName()) + "."
                            + this.getFormat().toString(), null);
    ...
    public FileEventProcessor getFileEP(URI dst, String title)
            throws MCASException {
        return new DashManifestManager(windowLength, segDuration, dst,
                getLevels(), this.getName(), title);
    }
